...
class HandPoseClassifier: NSObject, AVCaptureVideoDataOutputSampleBufferDelegate, ObservableObject {
    ...
    private let handPoseClassifier: MyHandPoseClassifierAug
    @Published var predictionResult: String?
    @Published var errorMessage: String?
    
    override init() {
        do {
            handPoseClassifier = try MyHandPoseClassifierAug(configuration: MLModelConfiguration())
            super.init()
            try setupAVSession()
        } catch {
    ...
    public func captureOutput(_ output: AVCaptureOutput, didOutput sampleBuffer: CMSampleBuffer, from connection: AVCaptureConnection) {
        ...
        do {
            ...
            do {
                ...
                let output: MyHandPoseClassifierAugOutput = try handPoseClassifier.prediction(poses: makeInputAttribute(recognizedPoints: handPoint))
    ...
    private func makeInputAttribute(recognizedPoints: [VNHumanHandPoseObservation.JointName : VNRecognizedPoint]) -> MLMultiArray {
        ...
        let mlArray = try! MLMultiArray(shape: [1, 3, 21], dataType: MLMultiArrayDataType.float32)
